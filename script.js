...
function mostrarPacientes(pacientes) {
  ...
  pacientes.forEach(p => {
    ...
    // Placas / CD / Informe: inputs solo si estado === Entregado
    ...
    // Estados: construyo select con control de bloqueo según reglas
    const estadoSelect = `
...
// =================== 🔹 Guardar campo (placas/cd/informe/entregado) ===================
function guardarCampo(key, campo, valor) {
  // No permitimos editar placas/cd/informe si estado es Atendido/Entregado? 
  // Estos inputs solo aparecen cuando estado === Entregado, así que basta actualizar.
...
function cambiarEstado(key, nuevoEstado) {
  ...
  // En atención sí puede volver a En espera o Programado -> permitimos cualquier cambio excepto hacia atrás desde Atendido/Entregado (ya cubierto)

  const fechaModificacion = new Date().toISOString().slice(0, 16);

  // si se pasa a Entregado: poner valores por defecto si faltan
...
// =================== 🔹 Firma digital (canvas con touch + repaint para móviles) ===================
...
// abrir modal solo si no hay firma (la UI ya muestra el botón solo si no hay firma)
...
// guardar firma en firebase (dataURL)
